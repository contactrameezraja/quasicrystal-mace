...
plateau and the $6Dt$ rise respectively. The displacement diagnostics average over twenty time origins at forty logarithmically spaced lags up to half the trajectory, and a curve counts as a plateau when the fitted slope over its second half, divided by the plateau value, lies below 0.02 per picosecond.
...
route reproduces. The nickel--nickel partial of Section~4.9 is sensitive to the nickel count, its peak height falling from 7.59 to 6.24 as the count rises from 24 to 81, which is why the 702-atom values are reported.
...
fixed-site validation is archived as its run log. A single swap evaluation timed at 15~ms for 26 atoms against 120~ms for a
position update; at the 3180-atom supercell, 780~ms per attempt set the
schedules of Section~3.10 at ten stages.

\subsection{Code listings}
\label{app:code}

The two routines on which the principal quantitative claims rest are
reproduced here verbatim from commit \texttt{239a1a5} (24 August 2026) of the
...
(retargeted box). The generation searches of Section~\ref{app:gen} are archived as their best structures (\texttt{mq\_*\_best.vasp}) alongside the per-stage traces (\texttt{mq\_*\_trace.npy}).
\clearpage
...
\texttt{make\_fig\_stacking\_}\allowbreak\texttt{centres.py} & the stacking gap-centre
...
\subsection{The generation searches}
\label{app:gen}
The searches summarised in Section~\ref{sec:gen} are reported here in full.

\subsubsection{The positional search fails, for an identifiable reason}
\label{app:gen-pos}
Five staged anneals from random positions in the fixed 26-atom cell reached at best 25.7, 38.8, 69.7, 65.9 and 51.6~meV per atom above the reference, five basins spanning 44~meV, and finished at 29.3, 42.4, 85.7, 93.8 and 51.6 (Figure~\ref{fig:gen}a), two having reached their best early, at stages 7 and 2, then drifted away, so single trajectories not only miss the minimum but fail to hold a basin. Replica exchange, the natural remedy, ran eight replicas over 1.2 million evaluations and 579 minutes to 56.1~meV above the reference, worse than the best anneal, with exchange acceptance of 0.44 to 0.60, the expected range and therefore not a tuning fault. Two explanations are consistent, a sampling volume roughly an order of magnitude below that of the published application~[13] and redrawing from the same distribution of frozen positional basins the anneals sampled.

\subsubsection{The chemical ordering is recovered exactly}
\label{app:gen-fixed}
With positions frozen on the reference site list and species randomised, the
same machinery succeeds completely, from 617~meV above the reference to the
reference value after relaxation (Section~4.3, Figure~\ref{fig:gen}b).
The entire 26 to 70~meV shortfall above is therefore the positional search's,
not the chemistry's or the potential's.
\begin{figure}[!ht]
\centering
\includegraphics[width=\textwidth]{fig_structure_search_validation.png}
\caption[The structure searches against the reference]{(a), the five staged anneals from random positions and the replica-exchange run against the reference energy. (b), the fixed-site search from randomised species, which recovers the reference exactly.}
\label{fig:gen}
\end{figure}
\FloatBarrier

\subsubsection{Tiling imposes no artificial chemistry}
\label{app:gen-tiling}
Fixed-site Monte Carlo on the 3180-atom supercell from the as-tiled arrangement, approximately 184\,000 swap attempts across schedules from 2500 down to 600~K, never moved from the as-tiled energy, acceptance falling to 0.003 to 0.008 with essentially every swap strongly uphill, and after relaxation the search's best differed from the untouched tiling by 0.002~meV per atom. The negative is trustworthy because the same machinery demonstrably finds better orderings when they exist (Section~\ref{app:gen-fixed}), and has a mechanism, the 265-atom chemistry itself deriving from fixed-site Monte Carlo with first-principles energies, so tiling inherits an already optimised ordering and the boundaries introduce no frustration worth relieving. The large boxes are therefore not detectably chemically strained by construction under the search applied; positions were frozen, so only a different arrangement on the same sites is excluded, not a different structure at the same composition.
