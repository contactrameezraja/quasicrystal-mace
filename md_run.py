...
-----
    ...
    # a structure file that is ALREADY the full box (no further tiling). float64
    # relaxation would exhaust GPU memory at this size, so relax in float32.
    python md_run.py --structure xbox_wcomp.vasp --supercell 1,1,1 --steps 100000 \
        --seed 33 --relax-dtype float32 --fmax 0.01 --out md_xbox_wcomp
"""
...
def make_calc(model, device, dtype):
    ...
    to v1-1M to match the O1 comparison."""
    if model == "mace":
        return mace_mp(model="medium-mpa-0", device=device, default_dtype=dtype)
    if model == "mattersim":
        from mattersim.forcefield import MatterSimCalculator   # lazy: only if used
...
def main():
    ...
    ap.add_argument("--relax-dtype", default="float64",
                    help="precision for the initial relaxation. float64 is right "
                         "when --structure is a small base cell that gets tiled "
                         "afterwards, but it exceeds 23 GB of GPU memory above "
                         "roughly 2000 atoms, so pass float32 when the input file "
                         "is already a large supercell.")
    ...
    ap.add_argument("--fix-cell", action="store_true",
                    help="relax positions only, holding the cell fixed. Use when "
                         ...
                         "relaxes the cell too, which is what every earlier run "
                         "in this project did.")
    ...
    # The dynamics need a well-converged starting point, so relax first, with
    # the SAME model as production: the O5 test wants each model's whole pipeline
    # sitting at its own minimum.
    #
    # Precision is a choice rather than a constant. float64 is preferable and is
    # the default, but it allocates several GB more than float32, and a
    # 3120-atom box overran a 22 GB card during relaxation (job 2147226). When
    # --structure is already a full supercell rather than a small base cell,
    # pass --relax-dtype float32 with --fmax 0.01, which is the precision the
    ...
    log(f"  cell lengths before {np.round(cell_before[:3], 3)} -> "
        f"after {np.round(base.cell.cellpar()[:3], 3)}")
